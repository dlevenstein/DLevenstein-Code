function [lfp,indices] = GetLFP_Down(channels,varargin)
...
%                   (default = 'id')
%     'downsample'  factor by which to downample by (default = 1), passed
%                   to LoadBinary_Down
%    =========================================================================
%
%  OUTPUT
...
if ~exist(filename,'file'),
    filename = [DATA.session.path '/' DATA.session.basename '.eeg'];
end
if ~exist(filename,'file'),
...
for i = 1:nIntervals,
	...
	start = intervals(i,1);
	% Load data
	data = LoadBinary_Down(filename,'duration',duration,...
        'frequency',DATA.rates.lfp,'nchannels',nChannels,'start',start,...
        'channels',channels,'downsample',downsamplefactor);
	t = start:(downsamplefactor/DATA.rates.lfp):(start+(length(data)-1)/(DATA.rates.lfp/downsamplefactor));
    t=t';
	lfp = [lfp ; t data];
